...
- [x] The admin can edit or delete the DM's (assistant-role) messages; players never can
- [x] A message is locked (no edit, no delete) once any DM reply exists later in the conversation than that message — this applies to everyone, including the admin, and to the DM's own messages (so in practice the admin can only touch the DM's latest reply)
...
- [x] An edited player message is visibly marked as edited, so the other player can tell its content changed after the fact. An edited DM message is not marked (2026-09-16): the admin's rewrite of a DM beat is meant to read as the DM's own words

## Decisions
- **Lock applies to everyone, including the admin.** Resolved via direct question — the alternative (admin override) risks leaving an existing DM reply referencing content that's since been changed or removed.
- **The admin may edit and delete DM messages, under the same lock** (2026-09-16). This replaced an earlier delete-only "admin can delete the single most recent message of any role" escape hatch: since a DM message with no later DM reply *is* the DM's latest reply, one rule covers both retrying a bad response (delete) and correcting one (edit), and the client mirror collapses to a single `canModify`. The `edited` flag is still set on DM rows; only the label is suppressed.
...
- **New `messages.edited` boolean**, defaulted false, set true on a successful edit. Drives the "edited" indicator in the UI (shown for player rows only). Deletes don't need an equivalent flag — the row is actually removed, not soft-deleted (matches how conversation deletion already works: real removal, no tombstone).

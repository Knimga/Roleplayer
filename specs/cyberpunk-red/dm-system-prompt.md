...
- Player identities are **not** hardcoded into the prompt file — the actual usernames from [users.js](../../packages/server-core/src/users.js) are appended at request time (e.g. "The two players are Evan and Nick"). This is what fixes the placeholder prompt inventing a fictional second player. (Path updated 2026-09-13 — this originally pointed at `server/config/users.js`, which moved to its current shared location during the monorepo restructure and later extraction into `packages/server-core`.)

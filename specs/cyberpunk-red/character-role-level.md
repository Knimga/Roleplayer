...
- [ ] Role choices: Rockerboy, Solo, Netrunner, Tech, Medtech, Media, Exec, Lawman, Fixer, Nomad
- [ ] Level must be an integer from 1 to 20 inclusive; the form cannot submit without a valid value for both fields, for both players
- [ ] Role and Level are locked once the Main Story is created — no way to edit afterward, same rule as the character name
- [ ] In a Main Story conversation's right panel, beneath the character name header (and its existing `<hr>`), the current user's Role and Level render as small gray italic text in the format "Level {level} {role}"
- [ ] This line is absent for non-Main-Story conversations, same as the character name header itself
- [ ] Existing Main Story conversations (created before this feature) are backfilled with placeholder Role/Level values so no conversation is left with null data

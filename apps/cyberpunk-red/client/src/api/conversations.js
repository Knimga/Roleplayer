...
// The URL still says "main-story" — that's the backend's route name, not
// user-facing terminology, and the two are intentionally allowed to diverge
// here. See product.md's Look & Feel note on the "Story" naming decision.
export async function createStory(characterNames, characterDetails) {
  const res = await fetch(`${API_BASE}/main-story`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    credentials: "include",

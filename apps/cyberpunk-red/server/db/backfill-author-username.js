...
// "user"` row. Non-Main-Story conversations already have the real username
// in `sender`, so those backfill directly. Main Story conversations display
...
async function run() {
  ...
  for (const conversation of allConversations) {
    ...
    for (const row of rows) {
      ...
      if (conversation.isMainStory && conversation.characterNames) {

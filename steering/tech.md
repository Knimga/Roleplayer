...
- Deliberately **not** shared, and shouldn't be: `schema.js`, `routes/conversations.js` (character-field-shaped), `DiceRoller.jsx`/`RightPanel.jsx`/the Hp/Sp/Ac trackers (game-specific mechanics — evaluated and declined: different field names, CSS ids, icons, and cyberpunk-red's HP tracker renders a Wound State line laria5e has no equivalent for), `MapModal.jsx`, `lib/claude.js` (per-app prompt assembly and model choice), `lib/enemyStats.js`/`lib/rollMessage.js` and each app's own combat system prompt (per-app combat mechanics — see `specs/combat-encounters.md` §6), all of `mcp/`, and every DM/combat system prompt.

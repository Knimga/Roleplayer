...
- Deliberately **not** shared, and shouldn't be: `schema.js` (game-specific data model), `routes/conversations.js` (character-field-shaped), `DiceRoller.jsx` / `RightPanel.jsx` / the Hp/Sp/Ac trackers / `NumberBarTracker.jsx` (game-specific mechanics), `Party.jsx` / `PartyMemberModal.jsx` / `MapModal.jsx` / `NewStoryModal.jsx`, `Sidebar.jsx` / `LeftPanel.jsx` (structurally near-identical but kept separate — differ in header branding), `lib/claude.js` (per-app system prompt assembly and model choice), all of `mcp/`, and the DM system prompts.
...
- Model: Cyberpunk Red uses `claude-sonnet-5` fixed. Laria 5e picks Sonnet in dev / Opus in prod automatically via `NODE_ENV`, overridable with `ANTHROPIC_MODEL`.

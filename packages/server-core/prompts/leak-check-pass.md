...
history, only the response text itself and the current active beat's
...
- The current active beat: `{title, narrative}`. This describes a
...
a specific secret fact from the beat's narrative — a name, identity, or
...
- If the response's own text shows players taking a concrete action that
  ...
  payoff of play, the exact reason the beat exists. Answer `false`.
...
- If the beat's narrative doesn't contain any specific, nameable secret

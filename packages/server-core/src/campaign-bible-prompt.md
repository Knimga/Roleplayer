...
The bible is written once, at campaign creation, using the player-provided Campaign Inputs below. It should very rarely change after creation — the things that change session-to-session (beat status, clock values, what's been discovered) live in a separate, smaller campaign_state record, not here. Write this document assuming it is close to permanent.
...
### 1. Campaign Inputs
Restate the player inputs verbatim for reference.
...
### 4. Arc Beats (narrative layer)
Write 3-5 beats marking the campaign's major waypoints, from the players' first unknowing contact with the central conflict through to its eventual confrontation or resolution. For each beat, use exactly this structure:

```
Beat ID: beat_<n>   (sequential: beat_1, beat_2, ...)
Title: <a few words>
Narrative: <what should be TRUE about the story once this beat is
    reached, and why it matters dramatically. Do NOT describe required
    actions, order of events, or specific scenes — describe a state of
    the story that can be reached by many different paths. Optionally
    include what tonal or emotional shift it represents>
```

Beat IDs must exactly match the beat tracker IDs used in the structured campaign_state record. Do not add extra fields or rename these labels — they are parsed by the app.
...
> - It's fine — expected — for the specific triggering condition of each beat to be defined separately in the operational beat-tracking data, not here. This section is about narrative meaning, not mechanics.
...
Write this plan in the following format:

```
Goal: <one sentence — what the antagonist is trying to achieve, in their
own terms, independent of the players>
```

Write 3-6 steps representing the antagonist's own plan toward that goal. Step 1 is something that is currently underway. Each MUST use exactly this structure:

```
Step: <n>   (sequential: 1, 2, 3...)
Description: <what this step accomplishes for the antagonist — a
    concrete move, not a vague escalation>
```

Write these as a sequence of moves a cunning actor would actually take, not a uniform ramp-up in danger. An early step might be quiet consolidation rather than an aggressive one.

> **IMPORTANT**: This plan is not fixed once written.
> - If players disrupt or expose a step, that will be marked in the structured record, not rewritten here.
> - The antagonist may then take a new step in reaction — these get appended with the next step number and reference which step they're responding to. Do not pre-write these reactions now; they only make sense in light of what players actually do.
> - Do not frame this plan as a ticking clock or fixed deadline. It should progress at whatever pace a thinking opponent actually would — pausing, accelerating, or changing shape based on circumstances, including player interference.

Do not include step status, awareness level, or resolution notes here — those are tracked separately in the structured villain_plan record.
